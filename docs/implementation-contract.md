...
- `GET /api/providers/moodle/courses/{id}/contents`: verify the course is in the connected account's enrolled list, then return sections `{id,name,summary,modules:[{id,name,type,url,description,subsectionId,resources:[{type,name,mimeType,size,modifiedAt,url,id,previewUrl,downloadUrl,previewKind}]}]}`. Names and descriptions are rendered as text. Activity links are generated on the configured Moodle origin; unsafe URLs and token-only file endpoints are omitted. New file fields are nullable, with `previewKind` equal to `pdf`, `image` or null. IDs derive from the private canonical file path and modification metadata, not a caller-selected URL or list index. For `type: "subsection"`, nullable `subsectionId` identifies the delegated section from Moodle `customdata.sectionid`. Only the ID is exposed; arbitrary custom data is never forwarded. Clients resolve it against the returned sections of this course, never by matching titles or by inventing a module URL. Missing/malformed optional metadata leaves the activity visible but explicitly unavailable. This endpoint returns metadata only and does not import files.
...
## Mobile content and owned document reader

- `GET /api/providers/moodle/courses/{courseId}/modules/{moduleId}` revalidates enrollment and resolves exactly one current module. It returns type/title/section, plain paragraph-preserving description/instructions/content, proxied resources, optional assignment metadata and submission requirements, plus explicit partial/warning state. It does not accept an arbitrary Moodle URL, assignment ID or target user ID.
- Assignment metadata uses `mod_assign_get_assignments` scoped to the authorized course. Only the selected module's current-user submission status is requested. Status permission failure preserves available descriptions and attachments. `cansubmit=false` is not interpreted as permission to upload/edit, and no write actions are exposed.
- Safe assignment attachments from introfiles/introattachments/activityattachments use the same opaque resource IDs and authenticated preview/download routes as course resources. Each file request resolves current authorized source metadata again; Moodle URLs and credentials never reach the mobile reader. Existing 32 MiB limits, timeouts, MIME/signature validation and connection-change guards remain.
- Moodle Page content uses `mod_page_get_pages_by_courses` with an exact course/module match. Server HTML parsing removes active elements and all attributes/URLs. Paragraphs remain; inline images are currently represented by alt-text hints, not a claim of full rich-page parity.
- `/reader.html` is a separate web build entry mounted by the shared native document Viewer. It validates a strict owned preview path and PDF/image kind, reuses the existing local PDF.js/image components, and provides loading/error/retry without external fallback links. Native navigation remains the surrounding app's responsibility.
- The reader has same-origin-only embedding and a restrictive CSP; normal app pages keep `X-Frame-Options: DENY`. The native host refuses navigation away from the exact reader URL, popups, downloads, file access and mixed content. PDF annotation navigation remains disabled. There is no external viewer service and no submission/upload implementation in this milestone.
