...
Every structure row has an always-visible checkbox immediately before its expand chevron for inclusion in the active learning structure. Nested source rows use the same left-side checkbox instead of the former eye action. Checked means included; unchecking a structure unit hides it and unchecking a source explicitly excludes that source. A separate **Ausgeblendete anzeigen** switch controls whether unchecked units and sources stay filtered from the tree or are revealed for restoration, so large courses can be reduced to the selected structure. **Alle auswählen** restores all structure units and clears explicit source exclusions; **Alle abwählen** hides all units and excludes all currently available owned sources. An entry hidden only through its parent stays unchecked and disabled until the parent is restored. Hiding and exclusion are reversible and do not open the options dialog.
