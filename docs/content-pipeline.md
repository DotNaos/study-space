...
The authoring boundary is source-owned: each reviewed source that contributes to a learning unit has one stable editable MDX block layered over its immutable imported source and machine extraction. The combined course/chapter view is a reducer over those source blocks in reviewed order. Human and agent edits create new editable revisions; they never rewrite the preserved source or extraction. A changed source revision marks the block stale until an explicit rebase/reset decision is made.
...
| Hierarchy and graph | Reviewed structure editor plus hierarchical source→target mapping board with reversible inherited hiding, explicit batch proposals, ordered source uses, responsive drill-down and CAS/history; existing graph and exact comparator retained. | Richer internal chapter proposals and graph visualization of the reviewed plan. |
...
| Script | Restricted inert MDX with Figure/TaskRef, reviewed unit order, source editing/preview/export and immutable candidates. The source-bound authoring foundation now persists one revisioned editable MDX block per reviewed source, deterministic extraction provenance, optimistic edits/reset, and stale-source detection without overwriting edits. | Combined block reducer/editor UI, PDF/current review workspace, automatic figure insertion, finer task-reference placement, and incremental rebase of independently edited content. |
