...
export function MappingRow({item,state,rootId,index,disabled,onMap,onExclude,onOpen,compact=false,selected=false}:{item:PipelineSourceView;state:PipelineState;rootId:string;index:number;disabled:boolean;onMap:(item:MappingItem)=>void;onExclude:(item:PipelineSourceView)=>void;onOpen:(item:PipelineSourceView)=>void;compact?:boolean;selected?:boolean}){
  ...
  return <>
    <li ref={setNodeRef} data-source-id={item.source.id} data-status={item.status} data-mapping={display.kind} data-role={use?.role} data-selected={selected||undefined} data-compact={compact||undefined} data-dragging={isDragging||undefined} style={{transform:CSS.Transform.toString(transform),transition}}>
      {compact&&!disabled&&<span className="mapping-selection-checkbox" title={included?"Ausblenden":"Verwenden"}>
      ...
      <button ref={compact?setActivatorNodeRef:undefined} className="mapping-source" type="button" onClick={()=>onOpen(item)} title={item.source.name} {...(compact&&!disabled?attributes:{})} {...(compact&&!disabled?listeners:{})}>
      ...
      {compact&&<button className="mapping-compact-role" type="button" onClick={()=>setEditing(true)} disabled={disabled} data-role={("role" in display&&display.role)||undefined} title={disabled?display.label:`${display.label} ändern`} aria-label={`${item.source.name}: ${display.label}`}><MappingIcon size={15}/></button>}
      ...
      {!compact&&<button className="mapping-eye" type="button" disabled={disabled} onClick={()=>onExclude(item)} aria-label={display.kind==="excluded"?`${item.source.name} wieder zuordnen`:`${item.source.name} ausblenden`} title={display.kind==="excluded"?"Wieder zuordnen":"Ausblenden"}>{display.kind==="excluded"?<Eye size={17}/>:<EyeOff size={17}/>}</button>}
      {!compact&&<button className="mapping-more" type="button" onClick={()=>onOpen(item)} aria-label={`${item.source.name} öffnen`} title="Öffnen"><MoreHorizontal size={18}/></button>}
    </li>
    {editing&&<MappingEditor item={item} state={state} rootId={rootId} index={index} disabled={disabled} onMap={onMap} onAdvanced={()=>{setEditing(false);onOpen(item);}} onClose={()=>setEditing(false)}/>}
  </>;
}
