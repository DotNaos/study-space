...
function MappingEditor({item,state,rootId,index,disabled,onMap,onAdvanced,onClose}:{
  ...
  return <DialogShell title="Zuordnung" onClose={onClose}>
    <div className="mapping-editor">
      <div className="mapping-editor-source"><FileText size={17}/><span>{item.source.name}</span></div>
...
function MappingRow({item,state,rootId,index,disabled,onMap,onExclude,onOpen}:{item:PipelineSourceView;state:PipelineState;rootId:string;index:number;disabled:boolean;onMap:(item:MappingItem)=>void;onExclude:(item:PipelineSourceView)=>void;onOpen:(item:PipelineSourceView)=>void}){
  ...
  const Icon=display.kind==="excluded"?EyeOff:display.kind==="open"?CircleAlert:roleIcon("role" in display?display.role:undefined);
  return <>
    <li ref={setNodeRef} data-source-id={item.source.id} data-status={item.status} data-mapping={display.kind} data-dragging={isDragging||undefined} style={{transform:CSS.Transform.toString(transform),transition}}>
      <button ref={setActivatorNodeRef} className="mapping-handle" type="button" disabled={!draggable||disabled} aria-label={`${item.source.name} sortieren`} {...attributes} {...listeners}><GripVertical size={15}/></button>
      <button className="mapping-source" type="button" onClick={()=>onOpen(item)} title={item.source.name}>
        <span className="mapping-source-icon"><FileText size={16}/></span><span><strong>{item.source.name}</strong></span>
      </button>
      ...
      <button className="mapping-value" type="button" onClick={()=>setEditing(true)} disabled={disabled} data-role={("role" in display&&display.role)||undefined} title={display.label}>
        <span className="mapping-value-icon"><Icon size={17}/></span><span><strong>{display.label}</strong>{display.kind==="proposal"&&<small>Vorschlag</small>}{item.status==="stale"&&<small>Erneut prüfen</small>}</span>
      </button>
...
function MappingDetail({courseId,state,rootId,onState,onOpenRoot,onOverview,onOpenSource,onCreate}:{courseId:number;state:PipelineState;rootId:string;onState:(state:PipelineState)=>void;onOpenRoot:(id:string)=>void;onOverview:()=>void;onOpenSource:(item:PipelineSourceView)=>void;onCreate:()=>void}){
  ...
  function exclude(item:PipelineSourceView){const display=displayMapping(state,item,rootId,items.indexOf(item));if(display.kind==="excluded"){const proposal=proposalFor(state,item,rootId,items.indexOf(item));if(proposal?.safe)map(proposal);else onOpenSource(item);}else map({sourceId:item.source.id,sourceVersion:item.source.sourceVersion,disposition:"exclude",uses:[]});}
