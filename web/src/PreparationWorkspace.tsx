...
import { ContentAuthoringView, type ContentSelection } from "./ContentAuthoringView";
...
export function PreparationWorkspace({
  ...
  const [selection, setSelection] = useState<ContentSelection>(() => firstUnit ? { kind: "unit", id: firstUnit.id } : { kind: "script" });
  const [editing, setEditing] = useState(false);

  useEffect(() => {
    if (selection.kind === "unit" && !state.units.some(unit => unit.id === selection.id && !unitHidden(unit, state.units))) {
      setSelection(firstUnit ? { kind: "unit", id: firstUnit.id } : { kind: "script" });
    }
    if (selection.kind === "source" && !state.sources.some(item => item.source.id === selection.id && item.source.present)) {
      setSelection(firstUnit ? { kind: "unit", id: firstUnit.id } : { kind: "script" });
    }
  }, [firstUnit, selection, state.sources, state.units]);
  ...
  return <section className="preparation-workspace" data-editing={editing || undefined}>
    <header className="preparation-workspace-head">
      <div>
        ...
        <span>{state.pending ? `${state.pending} Quellen offen` : "Struktur vollständig"}</span>
      </div>
      <button type="button" className="preparation-edit-toggle" aria-pressed={editing} onClick={() => setEditing(value => !value)}>
        {editing ? <Check size={14}/> : <PencilLine size={14}/>}<span>{editing ? "Fertig" : "Bearbeiten"}</span>
      </button>
    </header>

    <div className="preparation-workspace-panels">
      <aside className="preparation-structure-panel" aria-label="Struktur">
        <PipelineStructure
          state={state}
          busy={busy}
          onSave={onSave}
